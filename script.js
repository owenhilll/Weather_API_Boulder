...
//run when the document object model is ready for javascript code to execute
	...
	var url ='http://api.weatherstack.com/forecast?access_key=080c92caad7934352f0d97f704376a7e&query=40.0150,-105.2705'; //Place your weatherstack API Call Here
    ...
    */
    $(document).ready(function(){
         var url = 'http://api.weatherstack.com/forecast?access_key=080c92caad7934352f0d97f704376a7e&query=40.0150,-105.2705';
         	$.ajax({url:url, dataType:"jsonp"}).then(function(data){
         		var time = data.location.localtime;
         		document.getElementById('local_time').innerHTML = time;
         	     var Image = data.current.weather_icons;
			document.getElementById('image_today').innerHTML = Image;
         		var place = "Todays weather forecast: " + data.location.name;
         		document.getElementById('heading').innerHTML = place;
         		...
         		var precipitation = data.current.precip + "% chance of rain";
    ...
    /*
      ...
      */
			...
			for(var i = 1; i < 6;i++){
         			...
         			document.getElementById("5_day_forecast").innerHTML += '<div style="width: 20%;">' + '<div class="card">' + '<h5 class="card-title">'+ dayOfWeek 					+ '</h5>' + '<p class="card-text">High:'+ future.maxtemp + '<br>' + ' Low:'+ future.mintemp +'<br>' + ' Sunrise: ' + future.astro.sunrise 				+ '<br>' +' Sunset: ' + future.astro.sunset + ' <br> ' + '</div>' + '</div>' + '</div>';
         		}
